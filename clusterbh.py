...
class clusterBH:
    def __init__(self, N, rhoh, **kwargs):
        ...
        if (not (numpy.array_equal(self.a_slopes, self.a_kroupa) and numpy.array_equal(self.m_breaks,
                                                                                       self.m_kroupa))) and self.sev_tune and self.sev:
            ...
            # If the upper part of the IMF changes, tsev needs to change as well.
                ...
                warnings.warn(
                    'IMF slope > 1Msun is different than a Kroupa IMF, new stellar evolution rate cannot be automatically computed. '
        ...
        self.tidal_models = {
            ...
            'Exponential': lambda rh, rt: 3 * self.zeta / 5 * self.Rht * exp(self.c * rh / rt),
        ...
        self.cluster_model_dict = {
            # Dictionary for spherically symmetric cluster potentials. Units are [pc^2 / Myr^2]. Radius should be in [pc], mass in [Msun]. Currently used for for estimating the
        ...
        self.psi0 = self._psi(self.Mbh0 / (self.M0 + 0 * self.Mbh0), self.M0 + 0 * self.Mbh0, self.mbh0,
        ...
        if self.tidal:  # Valid check only if we have tides.
            if self.galactic_model in self.galactic_model_dict:
                ...
                self.dPhi_dr_func = self.galactic_model_dict[self.galactic_model][
                    'dPhi_dr']  # [pc/Myrs^2] Derivative of the galactic potential. Can be used to obtain the rotational curve for a galactic model at any distance.
                self.d2Phi_dr2_func = self.galactic_model_dict[self.galactic_model][
                    'd2Phi_dr2']  # [pc/Myrs^2] Derivative of the galactic potential. Can be used to obtain the rotational curve for a galactic model at any distance.
                self.Vc_func = lambda r: sqrt(1e3 * r * self.dPhi_dr_func(
                    r)) / 1.023  # [km / s] Velocity profile required to maintain a circular orbit.
                ...
                self.L = 1.023 * self.Vc_func(
                    self.rg) * self.rg * 1e3  # [pc^2 / Myrs] Orbital angular momentum at t=0. Conserved if no tidal spiralling is assumed.
        ...
        self.Phic_function = self.cluster_model_dict[self.cluster_model][
            'phi']  # [pc^2/Myrs^2] Potential of the cluster.
        self.dPhic_dr_func = self.cluster_model_dict[self.cluster_model][
            'dPhi_dr']  # [pc/Myrs^2] Derivative of the galactic potential.
    ...
    def _trh(self, M, rh, fbh, mbh, mst):
        ...
        trh = 0.138 * sqrt(M * rh ** 3 / self.G) / (mav * psi * log(
            self.gamma * Np))  # [Myrs]. Solution stops when the number of particles is low so the logarithm is well defined.
    ...
    def _odes(self, t, y):
        ...
        if self.tidal:  # Check if we have tides.
            ...
            if self.finite_escape_time:  # Check if evaporation is energy dependent.
                ...
                lambda1, lambda3 = Omega ** 2 - self.d2Phi_dr2_func(
                    RG), - Omega ** 2  # [1 / Myrs^2] Eigenvalues used for specifying the evaporation rate. Considers differences in galactic models.
        ...
        if self.mass_segregation:
            Mval_dot += Mval * (self.Mvalf - Mval) / trh * numpy.heaviside(tcc - t,
                                                                           0)  # [1 / Myrs] Evolution of parameter describing mass segregation.
        ...
        if self.tidal_spiralling and self.tidal:
            L = 1.023 * self.Vc_func(
                RG) * 1e3 * RG  # [pc^ / Myrs] Angular frequency of the orbit. At each point the angular frequency changes with radius.
    ...
    def _evolve(self, N, rhoh):
        ...
        sol = solve_ivp(self._odes, [0, self.tend], y, method=self.integration_method, t_eval=self.t_eval,
                        events=[event, tidal_overflow_event], rtol=self.rtol, atol=self.atol,
        ...
        if self.tidal:
            ...
            self.Ltot = 1.023 * self.Vc_func(
                self.RG) * 1e3 * self.RG if self.tidal else None  # [pc^2 / Myrs] Angular frequency of the model.
            ...
            self.tev = self._tev(self.M, self.rh, self.fbh, self.mbh,
                                 self.mst)  # [Myrs] Relaxation within rh. Describes evaporation.
            if self.tidal_spiralling: self.tdf = self._tdf(self.M, self.RG, self.Ltot,
                                                           self.rh)  # [Myrs] Dynamical friction time scale.
        ...
        self.trh = self._trh(self.M, self.rh, self.fbh, self.mbh,
                             self.mst)  # [Myrs] Relaxation within rh. Describes ejections.
        ...
        self.nu_run = numpy.maximum(self.nu_function(self.Z, sol.t),
                                    0) if self.sev else 0  # Running stellar mass loss rate.
