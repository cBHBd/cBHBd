...
class clusterBH:
    def __init__(self, N, rhoh, **kwargs):
        ...
        self.mseg_dict = {
            ...
            # First model evolves linearly in time but depends on core collapse.
    ...
    def _lambda(self, fbh):
        ...
        lambda_ = self.lambda_ if not self.lambda_exponent_run else self.lambda_min + (
                self.lambda_max - self.lambda_min) * (
                                                                            1 - fbh) ** self.lambda_exp  # Large fbh, starts with equal velocity dispersion. As time flows by it approaches equipartition.
    ...
    def _odes(self, t, y):
        ...
        eta_dot = mst_dot = 0  # [1/Myr, Msun/Myr] Derivatives for parameter of mass segregation, ratio rh / rv and average stellar mass.
        ...
        F = self.balance_function(
            t)  # Balancing function used to smoothly connect the two periods, (prior to post core collapse).
        ...
        if self.tidal:  # Check if we have tides.
            ...
            if self.escapers:  # It is connected to the model for the potential of the cluster.
                index = numpy.abs(self.Phic_function(rt, M)) / (
                        G * M)  # [pc^2/Myr^2 ] Get the value from the dictionary. It changes with respect to the potential assumed for the cluster.
        ...
        derivs = numpy.array([Mst_dot, Mbh_dot, rh_dot, mst_dot, Nst_dotsev, eta_dot, RG_dot],
    ...
    def _evolve(self, N, rhoh):
        ...
        y = [Mst[0], Mbh[0], rh[0], mst[0], Nst[0], eta[0], RG[0]]  # Combine them in a multivariable.
        ...
        def second_core_collapse_event(t, y):
            if self.BH:
                # Condition to track when the visible core is formed. Here defined when S drops below a critical value. Applies only if we form BHs.
                Mst, Mbh, mst = y[0], y[1], y[3]
        ...
        sol = solve_ivp(self._odes, [0, self.tend], y, method=self.integration_method, t_eval=self.t_eval,
                        events=[mass_event, particle_event, tidal_overflow_event, second_core_collapse_event],
                        rtol=self.rtol, atol=self.atol, dense_output=self.dense_output, vectorized=self.vectorize)
        ...
        self.eta = sol.y[5]  # Parameter for segregation.
        ...
        self.Edot_rlx = - self.zeta * self.E / self.trh  # [pc^2Msun/Myr^3] Energy production through relaxation.
        ...
        if self.dense_output:
            ...
            Edot_rlx_interp = interp1d(self.t, self.Edot_rlx)
            self.Edot_interp = lambda t: Edot_rlx_interp(t)
...
In future extension, the numer of elapsed relaxations will be used as an estimate for core collapse. This includes:
- A new differential equation. Say that Nrlx, eta, RG = y[5], y[6], y[7]    
- We include
