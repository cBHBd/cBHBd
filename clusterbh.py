...
class clusterBH:
    def __init__(self, N, rhoh, **kwargs):
        ...
        self.mseg_dict_t = {
            'linear': lambda t, trh, eta: self.aseg * (self.etaf - self.eta_min) * t ** (self.aseg - 1) / self.tcc ** self.aseg, # First model evolves linearly in time but depends on core collapse. If tcc=0 (fully segregated cluster) the elapsed relaxation is preferred for the linear model.
            'relaxation': lambda t, trh, eta: eta * (self.etaf - eta) / trh, # Second model evolves within one relaxation.
        }
        
        self.mseg_dict_rlx = {
            'linear': lambda Nrlx, Nrlx_dot, eta: self.aseg * (self.etaf - self.eta_min) * Nrlx_dot * Nrlx ** (self.aseg - 1),
        ...
        if not self.use_elapsed_relaxation:
            ...
            self.Nrlx0 * (1 - self.Sseg0) ** self.aseg0 # Initial segregation is constrained. It should affect self.eta0 as well (increase it from 3). 
    ...
    def _lambda(self, fbh):
        ...
        """
        lambda_ = self.lambda_ if not self.lambda_exponent_run else self.lambda_min + (self.lambda_max - self.lambda_min) * (1 - fbh) ** self.lambda_exp # Large fbh, starts with equal velocity dispersion. As time flows by it approaches equipartition if BHs are ejected first.
    ...
    def _psi(self, fbh, M, mbh, mst): 
        ...
        psi = self.a0  + self.a11 * self.a12 ** (1 + lambda_) * fbh ** self.b1 * (mbh / mav) ** ((1 + lambda_) * self.b2)  # A third term is needed for 3 component model (IMBHs).
    ...
    def _odes(self, t, y):
        ...
        F = self.balance_function(xq) # Balancing function used to smoothly connect the two periods, (prior to post core collapse).
        ...
        if self.tidal: # Check if we have tides. 
           ...
           index = - G * M / rt / self.Phic_function(rt, M) # [pc^2/Myr^2 ] Get the value from the dictionary. It changes with respect to the potential assumed for the cluster.
           
          
           if self.finite_escape_time: # Check if evaporation is energy dependent. Energetic stars leave faster.
              
              P = self.p * (tev * sqrt(rt_index / 2) * Omega) ** (1 - x)  # Check if we have finite escape time from the Lagrange points.
              
              xi *= P / (sqrt((1 - lambda3 / lambda1)) * index ** (3 / 2)) ** (1 - x) # Perhaps the denominator needs a factor of 3 / 4 so that the contribution is 1 for point mass?
           ...
           if Mst > mst_inf: # Tidal mass loss appears only when stars are present.                 
              ...
              rh_dot += 6 * xi / r / tev * (1 - tilde_E) * rh ** 2 / rt / index * Mst / M  / denom  # [pc/Myr] If escapers carry negative energy as they leave, the half-mass radius is expected to increase since it is similar to emitting positive energy. The effect is proportional to tides.
        ...
        if self.mass_segregation:
            eta_dot += self.mseg_func(yq, zq, eta) * numpy.heaviside(-xq, 0) # [1/Myr] Evolution of eta through derivative. 
        ...
        if self.tidal_spiralling and self.tidal:
            ...
            RG_dot -= 2 / (4 - self.rt_index(RG)) * RG / tdf * numpy.heaviside(RG - self.RG_crit, 0) # [kpc/Myr] Galactocentric distance cannot be negative. Truncate to the tidal radius for small values.
        ...
        derivs = numpy.array([Mst_dot, Mbh_dot, rh_dot, mst_dot, Nst_dotsev, Nrlx_dot, eta_dot, RG_dot], dtype=object) # Save all derivatives in a sequence.
    ...
    def _evolve(self, N, rhoh):
        ...
        if self.tidal: 
            ...
            self.cg_factor = 4 * self.cg / self.r * (self.rh / self.rt) ** 3 # Factor to account for the impact of the galactic potential to the energy production.
            if self.escapers:
                self.Edot_ev = - 6 * self.E / self.r * (1 - self.tilde_E) * self.rh * (- self.Phic_function(self.rt, self.M)) / (self.G * self.M) * self.xi / self.tev * self.Mst / self.M / (1 - 2 * self.cg_factor) # [pc^2Msun/Myr^3] Contribution of evaporated stars to the energy production.
            else:
        ...
        self.Edot_rlx = - self.zeta * self.E / self.trh * (1 + self.cg_factor) / (1 - 2 * self.cg_factor) # [pc^2Msun/Myr^3] Energy production through relaxation.
